...
Everything you record in Aura — attacks, pain levels, symptoms, medications,
check-in answers, notes, and settings — is stored in a local database on your
device. There is no account, no sign-in, no cloud copy, and no server of ours
...
in screens/settings/PrivacyScreen.tsx — the two must stay in sync. v1 ships
free: no purchase infrastructure runs or is contacted, so this section would
describe data processing that doesn't happen. Restore it (both copies, plus
the effective date above) in 1.1 when Aura Plus ships.
...
If a future version of Aura changes what happens with your data — for
example, an optional encrypted backup feature — this policy will be updated
